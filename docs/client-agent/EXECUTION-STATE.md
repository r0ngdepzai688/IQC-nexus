# IQC Nexus Client Agent — Durable Execution State & Work Directory Lifecycle (Phase 3A.5 Security Closure)

## Integration

`INascaExecutionStateStore` and `INascaWorkDirectoryManager` work together to manage job lifecycle:
- Work directories created under `%LocalAppData%\IqcQmsAgent\NascaWork\work_<32_hex_chars>`.
- `WorkDirectoryId` mapping is persisted in `NascaExecutionStateRecord` and `manifest.json`.
- Reparse points and junctions fail closed across all directory operations.
